...
const dateRanges = ref(null);
const currentPeriod = ref("CURRENT_MONTH");

onBeforeMount(() => {
  http.get("dates-range").then((response) => {
    dateRanges.value = response.data;
  });
});

function handleFilter(period) {
  currentPeriod.value = period;
  emits("selected", dateRanges[period]);
}
...
function handleCustomFilter() {
  emits("selected", [dateFilter.from, dateFilter.to]);
}
</script>
<template>
  <div class="dropdown filter-dropdown date-filter dropdown-menu-end">
    <a
      ...
      class="dropdown-toggle zf-daterange-picker d-flex align-items-center ember-view border p-1 rounded cursor-pointer"
