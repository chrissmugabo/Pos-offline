...
class OrderCreatedListener
{
    ...
    {
        $this->handleUsedStockQty($event->order, $event->items);
    }

    public function terminate(OrderCreatedEvent $event)
    {
        $this->handleUsedStockQty($event->order, $event->items);
    }
    ...
    private function handleUsedStockQty(Order $order, array $items)
    {
            ...
            foreach ($ingredients as $row) {
                ...
                if (!empty($item->old_quantity)) {
                    ...
                    DB::table('consumed_ingredients')->insert([
                        'date_consumed' => $order->created_at,
                        'item_id'   => $row->item_id,
       ...
       if(sizeof($outStockItems)) {
            $this->handleAutoRequisitions($order, $outStockItems, "STORABLE", "STOCK");
            $this->handleAutoRequisitions($order, $outStockItems, "DIRECT_USE", "PERISHABLES");
            $this->handleAutoRequisitions($order, $outStockItems, "PRODUCTION",  "PRODUCTION_STOCK");
        } 
    }
     ...
     * @return void
     */
    private function handleAutoRequisitions(Order $order, array $items, string $status, string $type)
    {
        ...
        if(count($finalItems)) {
            if($type == 'PRODUCTION_STOCK') {
                ...
                $productions = DB::table('item_sources')->whereNull('deleted_at')->get();
                foreach($productions as $production) {
                    ...
                    if(count($productionItems)) {
                        ...
                        $requisition = $this->getPendingRequistion($order, $type, $production->id);
                ...
                $requisition = $this->getPendingRequistion($order, $type);
                $requisition->total_amount += $amount;
                $requisition->save();
     ...
     * @return \Illuminate\Support\Collection
     */
    private function getPendingRequistion(Order $order, string $type, $destination =  NULL)
    {
        ...
        if(is_null($result)) {
            $id = Requisition::create([
                        ...
                        'create_user'  => $order->waiter_id ?? $order->create_user
                    ])->id;
